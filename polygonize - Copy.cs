namespace ArcSWAT3 {
                        ...
                        foreach (var i in Enumerable.Range(iBase + 1, this.rings.Count - (iBase + 1))) {
                            ...
                            if (ring != null) {
                                ...
                                if (lastv.x == firstv.x && lastv.y == firstv.y) {
                                    ...
                                    this.join(@base, ring, iBase, true);
            ...
            foreach (var i in Enumerable.Range(0, this.rings.Count)) {
                ...
                if (lastv.x == p1.x && lastv.y == p1.y) {
                    ...
                    if (firstv.x == p2.x && firstv.y == p2.y) {
                        ...
                        // note this direction test assumes links are either horizontal 
                        var second = ring[1];
    ...
    public class Shape {
        ...
        public class Offset {
            ...
            public MapPoint vertexToPoint(Vertex v) {
                var m = new MapPointBuilderEx(this.origin.X + this.dx * v.x, this.origin.Y - dy * v.y);
                return m.ToGeometry();
            }
            ...
            public ArcGIS.Core.Geometry.Polygon ringToPolygon(List<Vertex> ring) {
                //Convert a ring to a polygon with a single ring
                var mapPoints = (from v in ring select this.vertexToPoint(v)).ToList();
                var mb = new MultipointBuilderEx(mapPoints, AttributeFlags.None);
                var m = mb.ToGeometry();
                var pb = new PolygonBuilderEx(m);
                var p = pb.ToGeometry();
                return p;
            }

            public ArcGIS.Core.Geometry.Polygon ringsToPolygon(List<List<Vertex>> inrings) {
                //Convert a list of single ring Polygons to a Polygon
                var parts = (from inring in inrings select this.ringToPolygon(inring)).ToList();
                var pb = new PolygonBuilderEx(parts, AttributeFlags.None);
                return pb.ToGeometry();
            }

            public ArcGIS.Core.Geometry.Polygon polygonsToGeometry(List<Polygon> inpolys) {
                // Create a multi-polygon geometry from a list of polygons
                var polys = (from inpoly in inpolys select this.ringsToPolygon(inpoly.rings)).ToList();
                var pb = new PolygonBuilderEx(polys, AttributeFlags.None);
                return pb.ToGeometry();
            }

            public ArcGIS.Core.Geometry.Polygon makeGeometry(Shape shape) {
                //Make the geormtry from a shape
        ...
        public np.NDarray lastVals;
        public np.NDarray thisVals;
        public np.NDarray lastIds;
        public np.NDarray thisIds;
        public int rowNum;
        public int length;
        ...
        public Polygonize(bool connected4, int numCols, int noData, MapPoint p, double dX, double dY, StreamWriter fw =null) {
            this.lastVals = np.np.empty(new int[length]);
            np.Models.Shape npshape = new np.Models.Shape(new int[length]);
            this.thisVals = np.np.full(npshape, noData);
            this.lastIds = np.np.empty(new int[length]);
            this.thisIds = np.np.full(npshape, -1);
            this.rowNum = 0;
        ...
        public void addRow(np.NDarray row, int rowNum) {
            this.lastVals = np.np.copy(this.thisVals);
            this.lastIds = np.np.copy(this.thisIds);
            this.rowNum = rowNum;
            foreach (int i in Enumerable.Range(1, this.length - 1)) {
                int colNum = i - 1;
                bool thisIdDone = false;
                int thisVal = (int)this.thisVals[i];
                Shape shape;
                if (thisVal != this.noData) {
                    if (!this.shapes.TryGetValue(thisVal, out shape)) {
                        shape = new Shape(this.connected4, this.fw);
                    }
                    if (thisVal == (int)this.thisVals[i - 1]) {
                    ...
                    if (thisVal == (int)this.lastVals[i]) {
                        if (thisIdDone) {
                            shape.checkMerge((int)this.thisIds[i], (int)this.lastIds[i]);
                        } else {
                    ...
                    if (!this.connected4 && thisVal == (int)this.lastVals[i - 1]) {
                        if (thisIdDone) {
                            shape.checkMerge((int)this.thisIds[i], (int)this.lastIds[i - 1]);
                        } else {
                    ...
                    if (!thisIdDone) {
                        this.thisIds[i] = np.np.array(shape.newPoly());
            ...
            foreach (int i in Enumerable.Range(0, this.length - 1)) {
                int thisVal = (int)this.thisVals[i];
                int lastVal = (int)this.lastVals[i];
                int nextVal = (int)this.thisVals[i + 1];
                ...
                if (thisVal != lastVal) {
                    ...
                    v2.y = rowNum;
                    if (thisVal != this.noData) {
                        shape.addLink((int)this.thisIds[i], v1, v2); // r
                    }
                    if (lastVal != this.noData) {
                        if (this.connected4) {
                            this.shapes[lastVal].addLink((int)this.lastIds[i], v2, v1); // l
                        } else {
                            // defer adding left link 
                            leftLinks.Add((lastVal, (int)this.lastIds[i], v2, v1));
                        }
                    }
                }
                if (thisVal != nextVal) {
                    ...
                    v2.y = rowNum + 1;
                    if (thisVal != this.noData) {
                        shape.addLink((int)this.thisIds[i], v1, v2); // d
                    }
                    if (nextVal != this.noData) {
                        this.shapes[nextVal].addLink((int)this.thisIds[i + 1], v2, v1); // u
                    }
                }
            }
        ...
        public void finish() {
            ...
            foreach (int i in Enumerable.Range(1, this.length - 1)) {
                ...
                if (thisVal != this.noData) {
                    ...
                    v2.x = colNum;
                    v2.y = this.rowNum + 1;
                    this.shapes[thisVal].addLink((int)this.thisIds[i], v1, v2);
                }
            }
        ...
        public object getGeometry(int val) {
        ...
        public string shapesToString() {
            ...
            foreach (KeyValuePair<int, Shape> pair in this.shapes) {
                int val = pair.Key;
